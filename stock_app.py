...
app.layout = html.Div([
   ...
   # title
    html.H1("Crypto Stock Price Analysis", style={"textAlign": "center"}),
...
def update_price_graph(coin, price_type):
    predPrice = stock_price_prediction.predictByLSTM(coin, price_type, '2023-01-01', date.today())
    dataPrice = get_stock_data.getStockDataToNow(coin, 5*365)
    figure = {
        'data': [
            go.Scatter(
                x=dataPrice.index,
...
def update_volume_graph(coin):
    print('run')
    dataVolume = get_stock_data.getStockData(coin, '2018-01-01', '2023-01-01')
    figure = {
        'data': [
            go.Scatter(
                x=dataVolume.index,
